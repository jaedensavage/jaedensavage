...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            await message.reply_text(f"✅ Added **{user.mention}** as a Super User for katil OwO")
    ...
    if added:
        await message.reply_text(f"✅ Added **{mention}** as a Super User for katil OwO")
        return os.execvp("python3", ["python3", "-m", "Music"])
    await edit_or_reply(message, text="❌ Something wrong happened, check logs.")  
    return    
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if user.id not in await get_sudoers():
            return await message.reply_text(f"❌ Not a part of katil Sudo.")        
        removed = await remove_sudo(user.id)
        if removed:
            await message.reply_text(f"✅ Removed **{user.mention}** from katil Sudo.")
            return os.execvp("python3", ["python3", "-m", "Music"])
    ...
    if removed:
        await message.reply_text(f"✅ Removed **{mention}** from katil Sudo.")
        return os.execvp("python3", ["python3", "-m", "Music"])
    await message.reply_text(f"❌ Something wrong happened.")
